...
def trigger_deviation_score(trigger_x, sample_x, plain_model):
    ...
    for layer_name in origin_layer_value_dict.keys():
        ...
        origin_hist, origin_bins = np.histogram(origin_layer_value, bins=TENSEAL_BINS, range=(value_min, value_max))
        norm_origin_hist = (origin_hist + 1) / (np.sum(origin_hist) + TENSEAL_BINS)
        norm_weight = np.log(1 / norm_origin_hist)

        trigger_hist, trigger_bins = np.histogram(trigger_layer_value, bins=TENSEAL_BINS, range=(value_min, value_max))
        norm_trigger_hist = trigger_hist / np.sum(trigger_hist)
        layer_deviation_score = np.sum(norm_trigger_hist * norm_weight)
...
def find_trigger_gen_with_tenseal(data_name, src, dst, attack_op, cand_num=5):
    ...
    with open(f'./triggers/tenseal_{attack_op}({cand_num}_s{src}_d{dst})_{data_name}.pkl', 'wb') as fp:
        if attack_op == "nonsem":
...
if __name__ == "__main__":
    ...
    find_trigger_gen_with_tenseal(args.dataset, src=args.src, dst=args.dst, attack_op=args.func, cand_num=args.cand_num)